---
title: "PyTV - Python Tools for handling VCF files."
output: 
  html_document:
    keep_md: true
    toc: true
...
#### 3.1.1.2 Keep chromosomes 1 and 6 (multiple `--chr`)

```
python vcfFilter.py --vcf input.vcf --chr chr1 --chr chr6 --out output.vcf
```

#### 3.1.1.3 Keep all autosomes and chromosome X and Y ("-" and multiple `--chr`)

```
python vcfFilter.py --vcf input.vcf --chr chr1-chr22 --chr chrX --chr chrY --out output.vcf
```
...
```
python vcfFilter.py --vcf input.vcf --chr chr3 --chr chr6 --reverse --out output.vcf
```
