---
title: "pyVcfFilter - Filtering of variants in VCF file"
output: 
  html_document:
    keep_md: true
    toc: true
...
## 1.1 Variant Position Filtering

### 1.1.1 Chromosome filtering (`--chr`)
...
### 1.1.2 Region filtering (`--region` and `--region-file`)
...
### 1.1.3 Physical position filtering (`--phy-pos` and `--phy-pos-file`)
...
#### 1.1.3.2 Multiple positions were stored in a file
...
## 1.2 Variant ID Filtering (`--id` and `--id-file`)
...
## 1.3 Variant Type Filtering (`--keep-only-indels` and `--remove-indels`)
...
### 1.5.1 Remove variants wilth alternatice allele frequency higher than 0.01
...
### 1.5.2 Remove variants if the combined depth across samples is less than 200 

```
python pyVcfFilter.py --vcf example/input.vcf --info --key 'DP' --operator '>=' --value 200 --out example/output.vcf
```

### 1.5.3 Remove variants if the RMS mapping quality is less than 50

```
python pyVcfFilter.py --vcf example/input.vcf --info --key 'MQ' --operator '>=' --value 50 --out example/output.vcf
```

### 1.5.4 Remove variants located in the intergenic, downstream or upstream region
...
### 1.5.5 Remove variants wilth alternatice allele frequency in 1000 genomes higher than 0.05
...
### 1.5.6 Remove variants were predicted as 'D' by SIFT 
...
## 1.6 QUAL Field Filtering (`--qual`)

Filtering by **QUAL** field.  

### 1.6.1 Keep variants with phred-scaled quality score no less than 30

```
python pyVcfFilter.py --vcf example/input.vcf --qual 30 --out example/output.vcf
```

## 1.7 Allele Filtering

Filtering by **ALT** field. 

### 1.7.1 Minimum / maximum number of alleles
...
### 1.7.2 Biallelic / multiallelic site
...
## 1.9 Genotype Filtering

### 1.9.1 Missing rate and missing count filtering (`--missing-rate` and `--missing-count`)
...
### 1.9.2 Homozygous and heterozygous filtering (`--genotype`)
...
### 1.9.4 Compare genotype of multiple individuals

#### 1.9.4.1 Only variants have the same genotype across specified individuals will be kept
...
#### 1.9.4.2 Only variants have different genotype between the first individual and others will be kept
